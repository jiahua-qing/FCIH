...
parser.add_argument("--data_path", type=str, default="./data/merged_2020_2024-2.json")
...
valid_papers = [p for p in papers if 1800 <= paper_years[p] <= 2025]
print(f"Papers used for temporal splitting: {len(valid_papers)}/{len(papers)}")


train_papers = [p for p in valid_papers if 1800 <= paper_years[p] <= 2020]
val_papers = [p for p in valid_papers if 2021 <= paper_years[p] <= 2021]
test_papers = [p for p in valid_papers if 2022 <= paper_years[p] <= 2024]
...
train_pos_edges = filter_edges_by_year_fixed(positive_edges, paper_years, (2020, 2020))
val_pos_edges = filter_edges_by_year_fixed(positive_edges, paper_years, (2021, 2021))
test_pos_edges = filter_edges_by_year_fixed(positive_edges, paper_years, (2022, 2024))
...
def sample_negative_edges_fixed(
    ...
    while len(neg_set) < num_neg and attempts < max_total:
        ...
        if v in paper_years and paper_years[v] != 1800:
...
train_candidate_v = [p for p in papers if p in paper_years and 1800 <= paper_years[p] <= 2020]
...
val_candidate_v = [p for p in papers if p in paper_years and 1800 <= paper_years[p] <= 2021]
...
test_candidate_v = [p for p in papers if p in paper_years and 1800 <= paper_years[p] <= 2024]
...
train_cites_ctx = cites_edges_up_to(2020)
val_cites_ctx   = cites_edges_up_to(2020)
test_cites_ctx  = cites_edges_up_to(2020)
...
train_writes_ctx       = filter_edges_by_time_ctx(G, 2020, 'writes',       node_to_idx)
train_publication_ctx  = filter_edges_by_time_ctx(G, 2020, 'publication', node_to_idx)
...
val_writes_ctx       = filter_edges_by_time_ctx(G, 2021, 'writes',       node_to_idx)
val_publication_ctx  = filter_edges_by_time_ctx(G, 2021, 'publication', node_to_idx)
...
test_writes_ctx       = filter_edges_by_time_ctx(G, 2024, 'writes',       node_to_idx)
test_publication_ctx  = filter_edges_by_time_ctx(G, 2024, 'publication', node_to_idx)
...
h_dict_train, node_type_indices_train = build_phase_node_features_strict_with_tensor(2020, features_tensor)
h_dict_val,   node_type_indices_val   = build_phase_node_features_strict_with_tensor(2021, features_tensor)
h_dict_test,  node_type_indices_test  = build_phase_node_features_strict_with_tensor(2024, features_tensor)
